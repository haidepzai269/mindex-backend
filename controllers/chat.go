...
[END SHARED CONTEXT]`, systemPrompt, creatorName, *sharedSummary)
	...
	if req.CollectionID != "" {
		...
		query = `
			SELECT c.content, c.chunk_index, c.page_number, d.title AS doc_title, 1 - (c.embedding <=> $1::vector) AS similarity
